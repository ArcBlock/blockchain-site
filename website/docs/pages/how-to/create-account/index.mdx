...
description: Create Account with DeclareTx or other transactions
---

An account is a data entry that's stored in the blockchain ledger, there are several ways to create an account in the ledger.

## Create account with DeclareTx

The most intuitive way to create an account is to send a `DeclareTx` to the blockchain, here are a few examples:
...
const hash = await client.declare({ moniker: "blockchain-user", wallet });
```

The `moniker` field will be displayed on the account detail page in [blockchain explorer](https://beta.abtnetwork.io/explorer/accounts/z1TPn5rYHyxwbyphJ8ZoJECyVhy9tDDyEJU) like this:
...
const hash = await client.declare({
  ...
  data: {
    type: "json",
    value: {
      source: "hello-arcblock-blockchain",
    },
  },
  wallet,
});
...
ArcBlock blockchain also creates accounts implicitly in the following cases so that the user does not need to send `DeclareTx` ahead:
