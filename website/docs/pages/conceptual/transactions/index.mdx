...
To protect the blockchain from abuse, there are hard limits for each transaction size, transactions that exceed the size limit will fail at the verification phase.
...
The **Transaction Fee** is another mechanism for protecting the blockchain from attacks. There are 3 types of fees charged when a transaction is successfully executed:

- Protocol fee: charged when moving tokens with DepositTokenV2Tx and WithdrawTokenV2Tx across ArcBridge, fee rate may vary between bridges.
- Service fee: charged when sending `CreateAssetTx`, `CreateFactoryTx`, `CreateTokenTx` and `CreateRollupTx`.
...
ArcBlock blockchain has native support for multi-party-signed transactions. Transactions that support multi-party-signature includes:
