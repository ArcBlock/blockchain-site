...
- A wallet usually have a secret key (SK), a public key (PK) and an address ([DID](../did)).
- A wallet can be used to sign a [transaction](../transactions) or other format of message.
- A wallet can be used to verify a signed message.
- If you have the secret key you own the corresponding wallet.

## What is Account?

- An account is a data entry that exist in the blockchain ledger, each account is identified by a unique address ([DID](../did)).
...
- A user need at least one account before they can send and receive transactions on the blockchain.
